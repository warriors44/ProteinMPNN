...
def main(args: argparse.Namespace) -> None:
    ...
    if args.path_to_model_weights:
        ...
        k = file_path.rfind("/")
        model_folder_path = file_path[:k] + '/lo_model_weights/'
    ...
    # ---- Load JSON configs (identical to protein_mpnn_run.py) ----
    ...
    num_edges = checkpoint.get('num_edges', 64)
    noise_level_print = checkpoint.get('noise_level', args.backbone_noise)

    lo_kwargs = {}
    if 'num_samples' in checkpoint:
        lo_kwargs['num_samples'] = checkpoint['num_samples']
    else:
        lo_kwargs['num_samples'] = args.num_lo_samples
    ...
    if print_all:
        ...
        print(f'LO-ARM model loaded (separate_q_decoder={has_sep_q})')
    ...
    with torch.no_grad():
        for ix, protein in enumerate(dataset_valid):
            ...
            # ============================================================
            # Score-only mode (use IS with q for tighter estimate)
            # ============================================================
            if args.score_only:
                loop_c = 0
                ...
                for fc in range(1 + loop_c):
                    ...
                    if args.use_is_scoring:
                        loglik = model.compute_loglik_is_q(
                            X, S, mask, chain_M * chain_M_pos,
                            residue_idx, chain_encoding_all,
                            num_samples_eval=args.num_is_samples,
                        )
                        native_score = (-loglik).cpu().data.numpy()
                        native_score_list.append(native_score)
                        global_native_score_list.append(native_score)
                    else:
                        for j in range(NUM_BATCHES):
                            ...
                            native_score_list.append(scores.cpu().data.numpy())
                            global_scores = _scores(S, log_probs, mask)
                            global_native_score_list.append(
                                global_scores.cpu().data.numpy(),
                            )
                    ...
                    if print_all:
                        src = 'PDB' if fc == 0 else f'FASTA_{fc}'
                        print(
                            f'Score for {name_} from {src}, '
                            f'mean: {ns_mean_print}, std: {ns_std_print}, '
                            f'sample size: {ns_sample_size}, '
                            f'global score, mean: {global_ns_mean_print}, '
                            f'std: {global_ns_std_print}'
                        )

            # ============================================================
            # Conditional probs mode
            # ============================================================
            ...
            # ============================================================
            # Unconditional probs mode
            # ============================================================
            elif args.unconditional_probs_only:
                if print_all:
                    print(
                        f'Calculating sequence unconditional probabilities '
                        f'for {name_}'
                    )
            ...
            # ============================================================
            # Design mode: sample sequences with learned order
            # ============================================================
            else:
                randn_1 = torch.randn(chain_M.shape, device=X.device)
                log_probs = model(
                            ...
                            for b_ix in range(BATCH_COPIES):
                                ...
                                if b_ix == 0 and j == 0 and temp == temperatures[0]:
                                    ...
                                    try:
                                        ...
                                        commit_str = 'unknown'
                                    if args.ca_only:
                                        print_model_name = 'CA_LO_model_name'
                                    else:
                                        print_model_name = 'LO_model_name'
                                    f.write(
                                        '>{}, score={}, global_score={}, '
                                        'fixed_chains={}, designed_chains={}, '
                                        '{}={}, git_hash={}, seed={}\n{}\n'.format(
                                            name_, native_score_print,
                                            global_native_score_print,
                                            print_visible_chains,
                                            print_masked_chains,
                                            print_model_name, args.model_name,
                                            commit_str, seed, native_seq,
                                        )
                                    )
                                ...
                                decoding_order_str = ','.join(
                                    str(x) for x in sample_dict["decoding_order"][b_ix].cpu().numpy()
                                )
                                f.write(
                                    '>T={}, sample={}, score={}, global_score={}, '
                                    'seq_recovery={}, decoding_order=[{}]\n{}\n'.format(
                                        temp, sample_number, score_print,
                                        global_score_print, seq_rec_print,
                                        decoding_order_str, seq,
                                    )
                                )
                if args.save_score:
                    np.savez(
                ...
                if print_all:
                    print(
                        f'{num_seqs} sequences of length {total_length} '
                        f'generated in {dt} seconds'
                    )
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--ca_only", action="store_true", default=False, help="Parse CA-only structures and use CA-only models")
    argparser.add_argument("--path_to_model_weights", type=str, default="", help="Path to model weights folder")
    argparser.add_argument("--model_name", type=str, default="lo_48_020", help="LO-ARM ProteinMPNN model name")
    argparser.add_argument("--use_soluble_model", action="store_true", default=False, help="Flag to load weights trained on soluble proteins only")

    argparser.add_argument("--seed", type=int, default=0, help="If set to 0 then a random seed will be picked")

    argparser.add_argument("--save_score", type=int, default=0, help="0 for False, 1 for True; save score to npz files")
    argparser.add_argument("--save_probs", type=int, default=0, help="0 for False, 1 for True; save predicted probabilities per position")

    argparser.add_argument("--score_only", type=int, default=0, help="0 for False, 1 for True; score input backbone-sequence pairs")
    argparser.add_argument("--path_to_fasta", type=str, default="", help="Score provided input sequence in FASTA format")

    argparser.add_argument("--conditional_probs_only", type=int, default=0, help="0 for False, 1 for True; output conditional probabilities")
    argparser.add_argument("--conditional_probs_only_backbone", type=int, default=0, help="0 for False, 1 for True; conditional on backbone only")
    argparser.add_argument("--unconditional_probs_only", type=int, default=0, help="0 for False, 1 for True; output unconditional probabilities")
    ...
    argparser.add_argument("--sampling_temp", type=str, default="0.1", help="Space-separated sampling temperatures for amino acids")

    argparser.add_argument("--out_folder", type=str, help="Path to output folder")
    argparser.add_argument("--pdb_path", type=str, default='', help="Path to a single PDB to be designed")
    argparser.add_argument("--pdb_path_chains", type=str, default='', help="Define which chains to design for a single PDB")
    argparser.add_argument("--jsonl_path", type=str, help="Path to parsed pdb jsonl")
    argparser.add_argument("--chain_id_jsonl", type=str, default='', help="Path to chain ID dictionary")
    argparser.add_argument("--fixed_positions_jsonl", type=str, default='', help="Path to fixed positions dictionary")
    argparser.add_argument("--omit_AAs", type=list, default='X', help="Amino acids to omit from generation")
    argparser.add_argument("--bias_AA_jsonl", type=str, default='', help="Path to AA composition bias dictionary")

    argparser.add_argument("--bias_by_res_jsonl", default='', help="Path to per-position bias dictionary")
    argparser.add_argument("--omit_AA_jsonl", type=str, default='', help="Path to per-position AA omission dictionary")
    argparser.add_argument("--pssm_jsonl", type=str, default='', help="Path to PSSM dictionary")
    argparser.add_argument("--pssm_multi", type=float, default=0.0, help="PSSM weight [0.0, 1.0]")
    argparser.add_argument("--pssm_threshold", type=float, default=0.0, help="PSSM log-odds threshold")
    argparser.add_argument("--pssm_log_odds_flag", type=int, default=0, help="0 for False, 1 for True")
    argparser.add_argument("--pssm_bias_flag", type=int, default=0, help="0 for False, 1 for True")

    argparser.add_argument("--tied_positions_jsonl", type=str, default='', help="Path to tied positions dictionary")

    # LO-ARM specific arguments
    argparser.add_argument("--order_temperature", type=float, default=1.0, help="Temperature for order-policy sampling during design")
    argparser.add_argument("--num_lo_samples", type=int, default=2, help="Number of RLOO samples (only used if not in checkpoint)")
    argparser.add_argument("--num_is_samples", type=int, default=8, help="Number of importance samples for score_only mode with IS")
    argparser.add_argument("--use_is_scoring", action="store_true", default=False, help="Use importance-sampling with q_theta for scoring (tighter log-likelihood estimate)")
